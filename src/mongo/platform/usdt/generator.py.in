...
class Arg:
    def __init__(self, arg_dict, num_hits, probe_name, index, depth = 0):
        ...
        if arg_dict.get("value"):
            self.values = [arg_dict["value"]] * num_hits
        else:
            self.values = arg_dict["values"]

        if self.type == 'struct':
            self.values = [Arg(val, num_hits, probe_name, child_index, depth = depth + 1) for child_index, val in enumerate(self.values)]
